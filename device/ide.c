...
IdeChannel channels[2]; // 有两个ide通道
...
typedef struct __attribute__((packed)) partition_table_entry {
...
} PartitionTableEntry; // 保证此结构是 16 字节大小

// 引导扇区, mbr 或 ebr 所在的扇区
typedef struct __attribute__((packed)) boot_sector {
  uint8_t other[446];                     // 引导代码
  PartitionTableEntry partition_table[4]; // 分区表中有 4 项, 共 64 字节
...
static void partition_scan(Disk *hd, uint32_t ext_lba) {
  ...
  uint8_t part_idx = 0;
  PartitionTableEntry *p = bs->partition_table;
  ...
  while (part_idx++ < 4) {
    if (p->fs_type == 0x5) { // 若为扩展分区
      ...
      } else { // ext_lba_base 为 0 表示是第一次读取引导块,
               // 也就是主引导记录所在的扇区
