...
static int32_t copy_pcb_vaddrbitmap_stack0(TaskStruct *child_thread,
   ...
   */
  memcpy(vaddr_btmp, child_thread->userprog_vaddr.vaddr_map.bits,
         bitmap_pg_cnt * PG_SIZE);
  child_thread->userprog_vaddr.vaddr_map.bits = vaddr_btmp;
  /* 调试用 */
...
static void copy_body_stack3(TaskStruct *child_thread,
  ...
  uint8_t *vaddr_btmp = parent_thread->userprog_vaddr.vaddr_map.bits;
  uint32_t btmp_bytes_len =
      parent_thread->userprog_vaddr.vaddr_map.btmp_bytes_len;
...
static int32_t build_child_stack(TaskStruct *child_thread) {
  /* a 使子进程pid返回值为0 */
  /* 获取子进程0级栈栈顶 */
  IntrStack *intr_0_stack =
      (IntrStack *)((uint32_t)child_thread + PG_SIZE - sizeof(IntrStack));
  /* 修改子进程的返回值为0 */
  intr_0_stack->eax = 0;
