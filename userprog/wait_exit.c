...
static void release_prog_resource(TaskStruct *release_thread) {
  ...
  uint32_t bitmap_pg_cnt =
      (release_thread->userprog_vaddr.vaddr_map.btmp_bytes_len) / PG_SIZE;
  uint8_t *user_vaddr_pool_bitmap =
      release_thread->userprog_vaddr.vaddr_map.bits;
...
static bool find_child(ListElem *pelem, int32_t ppid) {
...
static bool find_hanging_child(ListElem *pelem, int32_t ppid) {
  TaskStruct *pthread = elem2entry(TaskStruct, all_list_tag, pelem);
...
/* list_traversal的回调函数,
 * 将一个子进程过继给init */
static bool init_adopt_a_child(ListElem *pelem, int32_t pid) {
  TaskStruct *pthread = elem2entry(TaskStruct, all_list_tag, pelem);
...
pid_t sys_wait(int32_t *status) {
  ...
  while (1) {
    /* 优先处理已经是挂起状态的任务 */
    ListElem *child_elem = list_traversal(&thread_all_list, find_hanging_child,
                                          parent_thread->pid);
    /* 若有挂起的子进程 */
    if (child_elem != NULL) {
