...
int32_t sys_mkdir(const char *pathname) {
  ...
  if (inode_no != -1) { // 如果找到了同名目录或文件, 失败返回
    ...
    // 先判断是否把 pathname 的各层目录都访问到了, 即是否在某个中间目录就失败了
    if (pathname_depth != path_searched_depth) { // 说明并没有访问到全部的路径,
                                                 // 某个中间目录是不存在的
  ...
  struct dir *parent_dir = searched_record.parent_dir;
  // 目录名称后可能会有字符 '/', 所以最好直接用 searched_record.searched_path,
  // 无 '/'
...
// 读取目录 dir 的 1 个目录项, 成功后返回其目录项地址, 到目录尾时或出错时返回
// NULL
...
// 在 inode 编号为 p_inode_nr 的目录中查找 indoe 编号为 c_inode_nr
// 的子目录的名字 将名字存入缓冲区 path 成功返回 0, 失败返回 -1
...
char *sys_getcwd(char *buf, uint32_t size) {
  ...
  ASSERT(child_inode_nr >= 0 &&
         child_inode_nr < 4096); // 最大支持 4096 个 inode
  // 若当前目录是根目录, 直接返回 '/'
  if (child_inode_nr == 0) {
    buf[0] = '/';
    buf[1] = 0;
  ...
  while ((last_slash = strrchr(full_path_reverse, '/'))) {
    ...
    // 在 full_path_reverse 中添加结束字符, 作为下一次执行 strcpy 中 last_slash
    // 的边界
    *last_slash = 0;
  }
  sys_free(io_buf);
